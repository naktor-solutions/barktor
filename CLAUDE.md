...
- **Build + package the .app (Xcode present):** `make app` → `dist/Barktor.app`; `make run` builds and launches it. Both sign with `SIGN_ID` (default `DEV_ID`, the Developer ID cert) + Hardened Runtime, and now **preflight-check the identity exists** — a fresh clone without that cert fails fast with an actionable message instead of a cryptic post-build `codesign` error.
- **Local dev loop (compile → replace the installed app):** `make install` builds, signs with the stable local self-signed cert (`LOCAL_SIGN_ID`, default `Barktor Local Dev`, no Hardened Runtime — library validation would reject the self-signed `llama.framework`), quits the running instance, and swaps the fresh bundle into `$(PREFIX)` (default `/Applications`). Signing with the *same* identity every rebuild is what keeps TCC grants alive; ad-hoc would re-prompt all three each time. It reuses the `app` target via `$(MAKE) app SIGN_ID=… SIGN_FLAGS=` rather than duplicating the bundle assembly. `make uninstall` quits and removes it. Override with `SIGN_ID=` / `PREFIX=` (e.g. `PREFIX=~/Applications` to skip sudo). CLT-only machines can't `make app`/`install`; use `scripts/release.sh`.
