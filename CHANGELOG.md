...
- **Transcribe in the background.** Meetings and dropped audio files now run through a background queue instead of holding you on a "Transcribing" pill — a menu-bar item shows live progress, and a notification (click it to reveal the result in History or Finder) arrives when each one finishes. Audio is saved to disk before transcription starts, so a job survives quitting mid-run and resumes on the next launch.
- **Drop audio files onto History to transcribe them.** Drag one or more audio files onto the History window and they're transcribed with your selected engine, landing in History like any dictation — the source file name is shown, and Retry and WAV export work as usual.
- **Expand a long transcript in History.** Rows keep a tidy three-line preview; a chevron expands one to its full text and collapses it back.
